...
let syncingRates = false

watch(agentRate, (value) => {
  if (syncingRates || value == null) return
  syncingRates = true
  companyRate.value = Math.max(0, Math.min(100, 100 - value))
  syncingRates = false
})

watch(companyRate, (value) => {
  if (syncingRates || value == null) return
  syncingRates = true
  agentRate.value = Math.max(0, Math.min(100, 100 - value))
  syncingRates = false
})
...
const persistChanges = async () => {
  ...
  try {
    await router.push(`/agents/${savedAgentId}`)
  } catch (error) {
